...
export class ValidationService {
   ...
   */
  validateFormSubmission(form: Form, responses: FieldResponse[]): string[] {
    const errors: string[] = [];
    ...
    form.fields.forEach((field) => {
      ...
      if (field.required) {
        if (!response) {
          errors.push(
            `Field "${field.label}" is required but no response was provided.`
          );
          return;
        }
        ...
        if (
          ...
          (!response.value || (response.value as string).trim() === "")
        ) {
          ...
          errors.push(
            `Field "${field.label}" is required but no option was selected.`
          );
        } else if (
          ...
          (!response.value || (response.value as string[]).length === 0)
        ) {
          errors.push(
            `Field "${field.label}" is required but no option was selected.`
          );
      ...
      if (response) {
        ...
        if (
          ...
          errors.push(
            `Field "${field.label}" requires a single selected option.`
          );
        } else if (
          field.type === FieldType.DROPDOWN &&
          typeof response.value !== "string"
        ...
        if (
          ...
          if (!validOptions.includes(response.value as string)) {
            errors.push(
              `Field "${field.label}" contains an invalid selection.`
            );
          }
        } else if (field.type === FieldType.CHECKBOX && field.options) {
          const validOptions = field.options.map((option) => option.id);
          if (
            !(response.value as string[]).every((value) =>
